...
export const MemberManager = ({
  ...
  const handleOpenEdit = (member) => {
    ...
    setFormData({
      ...
      assignedProjectName: member.assignedProjectName || '',
  ...
  const handleAssignProjectInline = async (memberId, projectId) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-extrabold text-slate-900 tracking-tight">Team Management &amp; Project Assignments</h2>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Assign team members to specific projects, designate <strong>Team Leads / Coordinators</strong>, and organize workforce structure.
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 mt-0.5">
                Leads &amp; Coordinators have <strong>view-only access</strong> to review workprogress, assign projects, &amp; download Excel for their assigned teammates.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {members.length > 0 ? (
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-1.5 text-xs text-slate-500">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{member.department || 'IT'}</span>
                    </div>
                  </div>

                  {/* Project Assignment Selector for this Member */}
                  <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                    <span className="text-[10px] uppercase font-bold text-slate-400 flex items-center gap-1">
                      <FolderKanban className="w-3 h-3 text-indigo-600" /> Assigned:
                    </span>
                    <select
                      value={member.assignedProjectId || ''}
                      onChange={(e) => handleAssignProjectInline(member.id, e.target.value)}
                      className="text-xs font-extrabold text-indigo-950 bg-indigo-50/70 border border-indigo-200 rounded-xl px-2.5 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-400 max-w-[170px] truncate cursor-pointer shadow-2xs"
                      title="Assign member to a specific project"
                    >
                ...
                {isLead && (
                  <div className="mt-4 pt-3 border-t border-amber-200/60 flex items-center justify-between">
                    <span className="text-xs font-extrabold text-amber-800 flex items-center gap-1">
                      ...
                      <strong className="font-mono ml-1">{assignedCount}</strong>
                    </span>
                    <button
                      onClick={() => handleOpenAssignModal(member)}
                      className="text-xs font-black text-amber-700 hover:text-amber-800 bg-amber-100/80 hover:bg-amber-200/80 px-2.5 py-1 rounded-xl transition-all cursor-pointer shadow-2xs"
                    >
                      Manage Team
                    </button>
                  </div>
          ...
          <div className="col-span-full py-12 text-center text-slate-400">
            No team members added yet. Click &quot;Add Member&quot; to create team accounts.
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:bg-white focus:ring-2 focus:ring-amber-500 focus:outline-none font-medium"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">Company Email</label>
                <input
                  type="email"
                  placeholder="e.g. john.doe@lionix.com"
              ...
              {/* Assigned Project Selection in Modal */}
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1">
                  Assigned Project (Default Project for Member)
                </label>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...
                      assignedProjectName: pObj ? pObj.name : ''
                    });
                  }}
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  When assigned, this project will automatically populate their hourly task sessions.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Selecting <strong>Team Lead</strong> or <strong>Team Coordinator</strong> gives them view-only progress, teammate project assignment, and Excel export access for their assigned team.
                </p>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-600">
              Select which employees report to <strong>{assigningLead.name}</strong>. {assigningLead.name} will have <strong>view-only access</strong> to inspect their hourly work progress, assign them to projects, and download their Excel work report.
            </p>
